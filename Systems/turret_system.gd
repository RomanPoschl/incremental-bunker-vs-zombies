...
func update(delta: float) -> void:
    for turret_id in turrets:
        ...
        if PlayerResources.get_ammo_count(turret.ammo_type) <= 0:
            continue
        ...
        if target_id != -1:
            if PlayerResources.spend_ammo(turret.ammo_type, 1):
...
func _fire_weapon(turret: TurretComponent, turret_id: int, target_id: int):
    print("Turret %s PEW PEW at Zombie %s! (Ammo left: %s)" %
        [turret_id, target_id, PlayerResources.get_ammo_count(turret.ammo_type)])
